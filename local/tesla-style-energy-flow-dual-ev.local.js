...
(function () {
  ...
  class EnergyFlowProCard extends HTMLElement {
      ...
      const vehicles = evSlots
        .map((slot) => {
          ...
          return {
            ...
            labelText: slot.key === 'ev2' ? 'EV 2' : 'EV 1',
            batteryText: !!batteryState ? `${Math.round(toPct(batteryState, 0))}%` : '--%'
          };
        })
        .filter((vehicle) => vehicle.configured);

      const totalPower = vehicles.reduce((sum, vehicle) => sum + vehicle.power, 0);
      const batteryLabels = vehicles
        .filter((vehicle) => vehicle.hasBatteryEntity)
        .map((vehicle) => `${Math.round(vehicle.battery)}%`);

      return {
        vehicles,
        totalPower,
        labelText: vehicles.length > 1 ? `${this._t('card.node.ev', 'EV')} x${vehicles.length}` : this._t('card.node.ev', 'EV'),
        batteryText: batteryLabels.length > 1 ? batteryLabels.join(' / ') : (batteryLabels[0] || '--%')
      };
    }
            ...
            <div class="scene">
              <svg viewBox="0 0 600 460" style="transform: scale(${sceneScale}); transform-origin: center;">
                ...
                <g class="flow-node" id="ev-node-group" transform="translate(184, 332)">
                  ...
                  <text class="flow-label" id="flow-ev-label" x="0" y="61">EV 1</text>
                ...
                <g class="flow-node" id="ev2-node-group" transform="translate(106, 316)">
    ...
    _renderDynamic() {
      ...
      const ev1 = evData.vehicles[0] || { power: 0, batteryText: '--%', labelText: 'EV 1', switchOn: false };
      const ev2 = evData.vehicles[1] || { power: 0, batteryText: '--%', labelText: 'EV 2', switchOn: false };
      if (evNodeGroup) {
        evNodeGroup.classList.toggle('ev-hidden', evHideIdle && !(ev1.power > 0 || ev1.switchOn));
      }
      if (ev2NodeGroup) {
        ev2NodeGroup.classList.toggle('ev-hidden', evHideIdle && !(ev2.power > 0 || ev2.switchOn));
      }
      ...
      this._setText('#flow-ev-label', ev1.labelText || 'EV 1');
  ...
  if (typeof window !== 'undefined') {
    window.TeslaStyleEnergyFlowLocalDefaults = {
      sceneComponentMap: JSON.parse(JSON.stringify(SCENE_FLOW_COMPONENT_MAP)),
      scenePathMap: JSON.parse(JSON.stringify(SCENE_FLOW_PATH_MAP))
    };
  }
  }
